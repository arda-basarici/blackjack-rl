...
- **Learning (why I'm building it):** RL literacy, and specifically *credit assignment* —
...
- **Portfolio (why it earns a place):** the **honesty thread**. Given only win/loss
  rewards, does an agent *rediscover* the proven-optimal basic-strategy table — and
...
- **Thread (why it's not a one-off):** this is the P3 link in the blackjack spine.
  P2 *computed* the optimum and analyzed the odds; here an agent *learns* which move is
  optimal; the planned P4 coach will *explain why* a move is optimal, using an LLM. This
...
- **A clean policy contract.** `strategies/base.py` defines `Strategy.decide(state) -> action`,
  ...
  `GameState` *already carries* counting and session fields (`true_count`,
  ...
  This is the single most important reuse decision: the comparison between *any* two
...
`hands_dealer_mirror`. These are not used to *train* the online agent; they are the raw
...
*across* hands as the deck depletes, and within one round there is nothing to count, so
...
the count and decides *how much to bet* across hands. The state grows (`true_count`,
...
tradeoff between expected value and risk of ruin, and the reward we optimize *encodes a
risk preference we chose*. So B's evaluation is necessarily fuzzier. The mature part of B
...
For readers new to RL: an *agent* observes a *state*, takes an *action*, receives a
*reward*, and the environment moves to a new state; the goal is a *policy* (state → action)
...
| Policy   | the strategy itself — and basic strategy *is* the optimal policy we're checking against |
...
by *folding the count and decks-remaining into the state*; if it didn't, B would become a
...
*represents nothing* there — a lookup table is exact and fully inspectable. Tabular Monte
Carlo control / Q-learning is the right-tool choice and keeps the policy readable cell by
cell. Reaching for a network *because it's impressive* would be the wrong-tool signal.
...
*comparable* precisely because they are graded by the same harness on the same metrics
...
**D4 — Model arc: tabular first, then a DQN as a *deliberate literacy experiment*.**
...
over-engineering trap. Instead we introduce the DQN *on Problem A, where ground truth
still exists*, and ask the sharpest possible question: **does function approximation
...
`hands_basic` (only the optimal action in each state) it can *imitate* but never learns
...
Splitting turns one episode into a *tree* (two hands, each its own trajectory whose
...
"complete."** Rationale is *isolation of variables*, not convenience: if splits were in
...
branching bookkeeping and ensuring rare split states get *visited* — an exploration
...
*atomic* — it plays a whole hand internally (calling `strategy.decide()` at each point) and
...
need to modify the engine. Instead the wrapper *captures episodes*: our agent (wrapped as a
...
episodes anyway, so this fits directly. The DQN learns from transitions *reconstructed* from
...
*would* require engine changes is genuine step-by-step interactive control, which our chosen
methods do not need. Offline training needs no env at all; it consumes the existing logs.
*Verified against `simulator/hand_simulator.py`, 2026-06-16.*

**D8 — Run persistence from day one: never overwrite; log *visit counts*.**
...
Inspection confirmed the engine is usable *as-is* for RL: `play_hand` returns trajectories
...
- **(a) Exploration failure** — rare, under-visited cells (e.g. splits, soft hands). A
  *fixable* shortfall: more episodes / better exploration closes it.
- **(b) Nothing to learn** — well-visited cells where two actions have near-equal EV and
  the reward signal genuinely *cannot* distinguish them. This is **not a failure**; it's an
...
- Phase 3 stays *lean* — literacy, not mastery; depth is reserved for Phase 4.
